...
$(document).ready(function() {
    ...
    $('#switchToEN').click(function(e) {
        e.preventDefault();
        window.location = "http://localhost:8080/en/index.html";
    });
    ...
    indexBook : function() {
        ...
        $.ajax({
            ...
            success: function (book) {
                $('#indexBookResult').empty();
                $('#indexBookResult').text("(Opciono) Podesi meta podatke...");
    ...
    configMetadata : function() {
        ...
        $.ajax({
            ...
            success : function(data) {
                $('#indexBookResult').empty();
                $('#indexBookResult').text("Meta podaci uspešno izmenjeni.");
                $('#setMetadata').hide();
            ...
            success : function(books) {
                ...
                $.each(books, function(index, book) {
                    $('#BooksResult').append(
                        ...
                        '<p>Datum nastanka: ' + book.createdDate + '</p>' +
                        '<p>Ključne reči: ' + book.keywords + '</p>' +
                        '<p class="show-inline">Preuzmi: </p><button value="' + book.id
                ...
                $('.bookIdDnw').click(function(e) {
                    ...
                    if($('#ProfileRole').val() == 'ADMINISTRATOR'){
                        ...
                        alert("Nije moguće preuzimanje. Pretplati se na izabranu kategoriju!")
                    }
                });
            }
        });
...
var BookManager = {
        ...
        $.ajax({
            ...
            success : function(books) {
                ...
                $.each(books, function(index, book) {
                    ...
                    if($('#ProfileRole').val() == 'ADMINISTRATOR'){
                        $('#BooksToShow').append(
                            '</p><button value="' + book.id + '" class="btn btn-danger bookIdDlt">Izbriši</button>' +
                            '</p><button value="' + book.id + '" class="btn btn-warning bookIdCng">Izmeni</button>')
                    }
                });
                $('.bookIdDnw').click(function(e) {
                    e.preventDefault();
    ...
    deleteBook : function(bookId, catId) {
        $.ajax({
            ...
            type : "DELETE",
            success : function() {
                alert("Knjiga je uspešno izbrisana.");
    ...
    checkSubscription : function(bookId) {
        $.ajax({
            ...
            error : function() {
                alert("Nisi pretplaćen na izabranu kategoriju.")
            }
        });
    },
...
var UserManager = {
    ...
    addUser : function() {
        $.ajax({
            ...
            error : function() {
                alert("Korisnik sa korisničim imenom: \"" + $('#UserUsername').val() + "\" već postoji!");
            }
        });
    },
...
var CategoryManager = {
    ...
    populateSubCatBox : function() {
        $.ajax({
            ...
            success: function(list) {
                ...
                $('#subCategoryBox').append('<option value="0">Sve</option>');
                $.each(list, function(index, category) {
            ...
            success : function(categories) {
                ...
                if($('#ProfileRole').val() == 'ADMINISTRATOR'){
                    $('#CategoriesList').append(
                        '<button id="btnNewCategory" class="btn btn-success">Nova kategorija</button></p>');
                }
                ...
                $('.catClass').click(function(e) {
                    ...
                    if($('#ProfileRole').val() == 'ADMINISTRATOR'){
                        $('#deleteCategoryTab').empty();
                        $('#deleteCategoryTab').append('<button class="btn btn-danger dltCategory">Izbriši kategoriju</button>');
    ...
    subscribeUser : function(username, catId) {
        $.ajax({
            ...
            success : function() {
                alert("Korisnik uspešno pretplaćen!");
                $('#SubUserUsername').val("");
            }
        });
    },
    ...
    populateQueryLangBox : function() {
        $.ajax({
            ...
            success: function(list) {
                ...
                $('#queryLanguageBox').append('<option value="0"></option>');
                $.each(list, function(index, lang) {
